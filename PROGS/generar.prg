...
*!*	 ENDIF
 ...
 IF m.ventas
    ...
    SELECT vistaventas.fecha, marcas.cuenta, almacenes.idfront AS sucursal, 000000000000000.00  AS debito, precio*unidadespagadas+dto*precio*unidadespagadas/100 AS credito, nit_varios AS tercero, 1, 2 AS naturaleza, 000000000000000.00  AS base, marcas.cc, "INTEFAZ ICG - VALOR ANTES DE IMPUESTOS" AS descripcio, .F. AS vtacredito, 000000 AS numfac, IIF(pu.codformapago='19', 010, almacenes.idfront) AS sucursale, pu.codformapago, cuantos FROM vistaVentas LEFT JOIN vistaArticulos ON vistaventas.codarticulo=vistaarticulos.codarticulo LEFT JOIN marcas ON vistaarticulos.marca=marcas.codmarca LEFT JOIN vistaSucursales ON vistaventas.caja=vistasucursales.cajamanager LEFT JOIN almacenes ON SUBSTR(vistasucursales.cajamanager, 1, 2)==almacenes.codalmacen LEFT JOIN pagoUnico AS pu ON vistaventas.fecha=pu.fecha AND vistaventas.numserie=pu.serie AND vistaventas.numalbaran=pu.numero WHERE tipoimpuesto<>3 AND SUBSTR(vistaventas.numserie, 4, 1)<>'I' AND precio*unidadespagadas<>0 AND vistaventas.codarticulo>0 UNION ALL SELECT vistaventas.fecha, impuestos.cuenta_ventas AS cuenta, almacenes.idfront AS sucursal, 000000000000000.00  AS debito, (total-total*dtocomercial/100)*vistaventas.iva/100 AS credito, nit_varios AS tercero, 2, 2 AS naturaleza, total-total*dtocomercial/100 AS base, "        " AS cc, "INTEFAZ ICG - IMPUESTOS               " AS descripcio, .F. AS vtacredito, 000000 AS numfac, IIF(pu.codformapago='19', 010, almacenes.idfront) AS sucursale, pu.codformapago, cuantos FROM vistaVentas LEFT JOIN vistaSucursales ON vistaventas.caja=vistasucursales.cajamanager LEFT JOIN impuestos ON vistaventas.tipoimpuesto=impuestos.tipoiva LEFT JOIN almacenes ON SUBSTR(vistasucursales.cajamanager, 1, 2)==almacenes.codalmacen LEFT JOIN pagoUnico AS pu ON vistaventas.fecha=pu.fecha AND vistaventas.numserie=pu.serie AND vistaventas.numalbaran=pu.numero WHERE tipoimpuesto<>3 AND SUBSTR(vistaventas.numserie, 4, 1)<>'I' AND LEN(ALLTRIM(caja))>0 UNION ALL SELECT vistapagos.fecha, formaspago.cuenta, almacenes.idfront AS sucursal, importe AS debito, 000000000000000.00  AS credito, IIF(LEN(ALLTRIM(formaspago.nit))>0, formaspago.nit, IIF(formaspago.credito, ALLTRIM(IIF(ISNULL(cif), "", cif)), nit_varios)) AS tercero, 3, 1 AS naturaleza, 000000000000000.00  AS base, formaspago.cc AS cc, "INTEFAZ ICG - PAGOS                   " AS descripcio, formaspago.credito AS vtacredito, IIF(formaspago.credito, IIF(formaspago.numerofiscal, vistapagos.numerofiscal, getconsecutivo2(vistapagos.fecha)), 000000) AS numfac, IIF((pu.codformapago='19' AND cuantos=1) OR (pu.codformapago='19' AND cuantos>1 AND importe>0), 010, almacenes.idfront) AS sucursale, pu.codformapago, cuantos FROM vistaPagos LEFT JOIN formasPago ON vistapagos.codformapago=formaspago.codformapago LEFT JOIN vistaSucursales ON vistapagos.caja=vistasucursales.cajamanager LEFT JOIN almacenes ON SUBSTR(vistasucursales.cajamanager, 1, 2)==almacenes.codalmacen LEFT JOIN pagoUnico AS pu ON vistapagos.fecha=pu.fecha AND vistapagos.serie=pu.serie AND vistapagos.numero=pu.numero WHERE SUBSTR(vistapagos.serie, 4, 1)<>'I' AND LEN(ALLTRIM(caja))>0 AND vistapagos.codtipopago IS NOT NULL  UNION ALL SELECT vistaventas.fecha, m.cuenta_exentas AS cuenta, almacenes.idfront AS sucursal, 000000000000000.00  AS debito, precio*unidadespagadas AS credito, nit_varios AS tercero, 4, 2 AS naturaleza, 000000000000000.00  AS base, marcas.cc, "INTEFAZ ICG - EXENTOS                 " AS descripcio, .F. AS vtacredito, 000000 AS numfac, IIF(pu.codformapago='19', 010, almacenes.idfront) AS sucursale, pu.codformapago, cuantos FROM vistaVentas LEFT JOIN vistaArticulos ON vistaventas.codarticulo=vistaarticulos.codarticulo LEFT JOIN marcas ON vistaarticulos.marca=marcas.codmarca LEFT JOIN vistaSucursales ON vistaventas.caja=vistasucursales.cajamanager LEFT JOIN almacenes ON SUBSTR(vistasucursales.cajamanager, 1, 2)==almacenes.codalmacen LEFT JOIN pagoUnico AS pu ON vistaventas.fecha=pu.fecha AND vistaventas.numserie=pu.serie AND vistaventas.numalbaran=pu.numero WHERE tipoimpuesto=3 AND SUBSTR(vistaventas.numserie, 4, 1)<>'I' AND precio*unidadespagadas<>0 UNION ALL SELECT vistaventas.fecha, m.cuenta_descuentos AS cuenta, almacenes.idfront AS sucursal, dto*precio*unidadespagadas/100 AS debito, 000000000000000.00  AS credito, nit_varios AS tercero, 5, 1 AS naturaleza, 000000000000000.00  AS base, marcas.cc, "INTEFAZ ICG - DESCUENTOS              " AS descripcio, .F. AS vtacredito, 000000 AS numfac, IIF(pu.codformapago='19', 010, almacenes.idfront) AS sucursale, pu.codformapago, cuantos FROM vistaVentas LEFT JOIN vistaArticulos ON vistaventas.codarticulo=vistaarticulos.codarticulo LEFT JOIN marcas ON vistaarticulos.marca=marcas.codmarca LEFT JOIN vistaSucursales ON vistaventas.caja=vistasucursales.cajamanager LEFT JOIN almacenes ON SUBSTR(vistasucursales.cajamanager, 1, 2)==almacenes.codalmacen LEFT JOIN pagoUnico AS pu ON vistaventas.fecha=pu.fecha AND vistaventas.numserie=pu.serie AND vistaventas.numalbaran=pu.numero WHERE dto<>0 AND SUBSTR(vistaventas.numserie, 4, 1)<>'I' UNION ALL SELECT descuentoscomerciales.fecha, m.cuenta_descuentos AS cuenta, almacenes.idfront AS sucursal, SUM(totdtocomercial) AS debito, 000000000000000.00  AS credito, nit_varios AS tercero, 6, 1 AS naturaleza, 000000000000000.00  AS base, "30080   " AS cc, "INTEFAZ ICG - DESCUENTOS              " AS descripcio, .F. AS vtacredito, 000000 AS numfac, IIF(pu.codformapago='19', 010, almacenes.idfront) AS sucursale, pu.codformapago, cuantos FROM descuentosComerciales LEFT JOIN vistaSucursales ON descuentoscomerciales.caja=vistasucursales.cajamanager LEFT JOIN almacenes ON SUBSTR(vistasucursales.cajamanager, 1, 2)==almacenes.codalmacen LEFT JOIN pagoUnico AS pu ON descuentoscomerciales.fecha=pu.fecha AND descuentoscomerciales.numserie=pu.serie AND descuentoscomerciales.numalbaran=pu.numero WHERE SUBSTR(descuentoscomerciales.numserie, 4, 1)<>'I' AND totdtocomercial<>0 GROUP BY descuentoscomerciales.fecha, almacenes.idfront, sucursale, pu.codformapago INTO CURSOR ventas1
    SELECT * FROM ventas1 UNION ALL SELECT vistapropinas.fecha, m.cuenta_credito_propinas AS cuenta, almacenes.idfront AS sucursal, 000000000000000.00  AS debito, propina AS credito, nit_varios AS tercero, 7, 2 AS naturaleza, 000000000000000.00  AS base, "        " AS cc, "INTEFAZ ICG - PROPINAS                " AS descripcio, .F. AS vtacredito, 000000 AS numfac, IIF(pu.codformapago='19', 010, almacenes.idfront) AS sucursale, pu.codformapago, cuantos FROM vistaPropinas LEFT JOIN almacenes ON SUBSTR(vistapropinas.caja, 1, 2)==almacenes.codalmacen LEFT JOIN pagoUnico AS pu ON vistapropinas.fecha=pu.fecha AND vistapropinas.numserie=pu.serie AND vistapropinas.numfactura=pu.numero WHERE propina<>0 UNION ALL SELECT vistapropinas.fecha, m.cuenta_debito_propinas AS cuenta, almacenes.idfront AS sucursal, propina AS debito, 000000000000000.00  AS credito, nit_varios AS tercero, 7, 1 AS naturaleza, 000000000000000.00  AS base, "        " AS cc, "INTEFAZ ICG - PROPINAS                " AS descripcio, .F. AS vtacredito, 000000 AS numfac, IIF(pu.codformapago='19' AND cuantos=1, 010, almacenes.idfront) AS sucursale, pu.codformapago, cuantos FROM vistaPropinas LEFT JOIN almacenes ON SUBSTR(vistapropinas.caja, 1, 2)==almacenes.codalmacen LEFT JOIN pagoUnico AS pu ON vistapropinas.fecha=pu.fecha AND vistapropinas.numserie=pu.serie AND vistapropinas.numfactura=pu.numero WHERE propina<>0 UNION ALL SELECT vistaventas.fecha, m.cuenta_costos_debito AS cuenta, almacenes.idfront AS sucursal, SUM(unidadespagadas*ROUND(coste, 0)) AS debito, 000000000000000.00  AS credito, nit_varios AS tercero, 8, 1 AS naturaleza, 000000000000000.00  AS base, marcas.cc, "INTEFAZ ICG - COSTOS                  " AS descripcio, .F. AS vtacredito, 000000 AS numfac, IIF(pu.codformapago='19', 010, almacenes.idfront) AS sucursale, pu.codformapago, cuantos FROM vistaVentas LEFT JOIN vistaArticulos ON vistaventas.codarticulo=vistaarticulos.codarticulo LEFT JOIN marcas ON vistaarticulos.marca=marcas.codmarca LEFT JOIN vistaSucursales ON vistaventas.caja=vistasucursales.cajamanager LEFT JOIN almacenes ON SUBSTR(vistasucursales.cajamanager, 1, 2)==almacenes.codalmacen LEFT JOIN pagoUnico AS pu ON vistaventas.fecha=pu.fecha AND vistaventas.numserie=pu.serie AND vistaventas.numalbaran=pu.numero GROUP BY vistaventas.fecha, almacenes.idfront, sucursale, pu.codformapago UNION ALL SELECT vistaventas.fecha, m.cuenta_costos_credito AS cuenta, almacenes.idfront AS sucursal, 000000000000000.00  AS dedito, SUM(unidadespagadas*ROUND(coste, 0)) AS crebito, nit_varios AS tercero, 9, 2 AS naturaleza, 000000000000000.00  AS base, marcas.cc, "INTEFAZ ICG - COSTOS                  " AS descripcio, .F. AS vtacredito, 000000 AS numfac, IIF(pu.codformapago='19', 010, almacenes.idfront) AS sucursale, pu.codformapago, cuantos FROM vistaVentas LEFT JOIN vistaArticulos ON vistaventas.codarticulo=vistaarticulos.codarticulo LEFT JOIN marcas ON vistaarticulos.marca=marcas.codmarca LEFT JOIN vistaSucursales ON vistaventas.caja=vistasucursales.cajamanager LEFT JOIN almacenes ON SUBSTR(vistasucursales.cajamanager, 1, 2)==almacenes.codalmacen LEFT JOIN pagoUnico AS pu ON vistaventas.fecha=pu.fecha AND vistaventas.numserie=pu.serie AND vistaventas.numalbaran=pu.numero GROUP BY vistaventas.fecha, almacenes.idfront, sucursale, pu.codformapago UNION ALL SELECT vencabezado.fecha, m.cuenta_db_cree AS cuenta, almacenes.idfront AS sucursal, m.tasa_cree*totalbruto/100 AS debito, 000000000000000.00  AS credito, nit_varios AS tercero, 1, 1 AS naturaleza, 000000000000000.00  AS base, "        " AS cc, "INTEFAZ ICG - AUTORETENCION CREE" AS descripcio, .F. AS vtacredito, 000000 AS numfac, IIF(pu.codformapago='19', 010, almacenes.idfront) AS sucursale, pu.codformapago, cuantos FROM vencabezado LEFT JOIN vistaSucursales ON vencabezado.caja=vistasucursales.cajamanager LEFT JOIN almacenes ON SUBSTR(vistasucursales.cajamanager, 1, 2)==almacenes.codalmacen LEFT JOIN pagoUnico AS pu ON vencabezado.fecha=pu.fecha AND vencabezado.numserie=pu.serie AND vencabezado.numalbaran=pu.numero UNION ALL SELECT vencabezado.fecha, m.cuenta_cr_cree AS cuenta, almacenes.idfront AS sucursal, 000000000000000.00  AS debito, m.tasa_cree*totalbruto/100 AS credito, nit_varios AS tercero, 1, 2 AS naturaleza, 000000000000000.00  AS base, "        " AS cc, "INTEFAZ ICG - AUTORETENCION CREE" AS descripcio, .F. AS vtacredito, 000000 AS numfac, IIF(pu.codformapago='19', 010, almacenes.idfront) AS sucursale, pu.codformapago, cuantos FROM vencabezado LEFT JOIN vistaSucursales ON vencabezado.caja=vistasucursales.cajamanager LEFT JOIN almacenes ON SUBSTR(vistasucursales.cajamanager, 1, 2)==almacenes.codalmacen LEFT JOIN pagoUnico AS pu ON vencabezado.fecha=pu.fecha AND vencabezado.numserie=pu.serie AND vencabezado.numalbaran=pu.numero INTO CURSOR ventas
 ...
 ENDIF
 IF m.inventarios
    SELECT fecha, almacenes.idfront AS sucursal, almacenes.idfront AS sucursale, m.cuenta_inventarios AS cuenta, 000000000000000.00  AS debito, (stock-unidades)*precio AS credito, m.nit AS tercero, 2 AS naturaleza, marcas.cc AS cc FROM vistaInventarios AS inven LEFT JOIN almacenes ON inven.codalmacenorigen=almacenes.codalmacen LEFT JOIN marcas ON inven.marca=marcas.codmarca WHERE unidades<stock AND ((codalmacenorigen='A1' AND codmarca=4) OR codmarca<>4) UNION ALL SELECT fecha, almacenes.idfront AS sucursal, almacenes.idfront AS sucursale, m.cuenta_costo_ventas AS cuenta, (stock-unidades)*precio AS debito, 000000000000000.00  AS credito, m.nit AS tercero, 1 AS naturaleza, marcas.cc AS cc FROM vistaInventarios AS inven LEFT JOIN almacenes ON inven.codalmacenorigen=almacenes.codalmacen LEFT JOIN marcas ON inven.marca=marcas.codmarca WHERE unidades<stock AND ((codalmacenorigen='A1' AND codmarca=4) OR codmarca<>4) UNION ALL SELECT fecha, almacenes.idfront AS sucursal, almacenes.idfront AS sucursale, m.cuenta_inventarios AS cuenta, (unidades-IIF(stock>=0, stock, 0))*precio AS debito, 000000000000000.00  AS credito, m.nit AS tercero, 1 AS naturaleza, marcas.cc AS cc FROM vistaInventarios AS inven LEFT JOIN almacenes ON inven.codalmacenorigen=almacenes.codalmacen LEFT JOIN marcas ON inven.marca=marcas.codmarca WHERE unidades>stock AND ((codalmacenorigen='A1' AND codmarca=4) OR codmarca<>4) UNION ALL SELECT fecha, almacenes.idfront AS sucursal, almacenes.idfront AS sucursale, m.cuenta_costo_ventas AS cuenta, 000000000000000.00  AS debito, (unidades-IIF(stock>=0, stock, 0))*precio AS credito, m.nit AS tercero, 2 AS naturaleza, marcas.cc AS cc FROM vistaInventarios AS inven LEFT JOIN almacenes ON inven.codalmacenorigen=almacenes.codalmacen LEFT JOIN marcas ON inven.marca=marcas.codmarca WHERE unidades>stock AND ((codalmacenorigen='A1' AND codmarca=4) OR codmarca<>4) INTO CURSOR inventarios
    BROWSE NOAPPEND NOEDIT NODELETE NORMAL
 ENDIF
 IF m.bajas
 ...
 IF m.ventas
    ...
    IF RECCOUNT()>0
       ...
       ENDSCAN
       SELECT "RV" AS tipodocumento, consecutiv AS numerodoc, ventas.fecha, ventas.cuenta, ventas.sucursal, ROUND(SUM(debito), 0) AS debito, ROUND(SUM(credito), 0) AS credito, ROUND(SUM(base), 0) AS base, tercero, cc, descripcio, vtacredito, numfac, ventas.sucursale FROM ventas LEFT JOIN consecutivos ON ventas.fecha=consecutivos.fecha AND ventas.sucursal=consecutivos.sucursal WHERE debito<>0 OR credito<>0 GROUP BY ventas.fecha, tipodocumento, cuenta, descripcio, tercero, ventas.sucursal, naturaleza, cc, numfac, ventas.sucursale ORDER BY ventas.fecha, tipodocumento, numerodoc INTO CURSOR c1
       copiar3("c1")
       SELECT consecutivos
       USE
    ENDIF
 ...
 SCAN
    ...
    IF entro
       IF ABS(m.debitos-m.creditos)>m.error
          WAIT WINDOW "La diferencia entre débitos y créditos excede error permitido "+TRANSFORM(m.debitos-m.creditos, "999,999,999.99")+" en el documento "+ALLTRIM(m.tipodocumento)+"-"+ALLTRIM(STR(m.numerodoc))
          BROWSE NOAPPEND NOEDIT NODELETE NORMAL
       ELSE
 ...
 SCAN
    ...
    IF entro
       ...
       ELSE
          IF ABS(m.debitos-m.creditos)>0
             IF m.debitos>m.creditos
                LOCATE ALL FOR ALLTRIM(m.tipodocume)==ALLTRIM(tipodocume) .AND. credito<>0 .AND. numerodoc=m.numerodoc .AND. sucursale=m.sucursale
                REPLACE credito WITH credito+(m.debitos-m.creditos)
             ELSE
                LOCATE ALL FOR ALLTRIM(m.tipodocume)==ALLTRIM(tipodocume) .AND. debito<>0 .AND. numerodoc=m.numerodoc .AND. sucursale=m.sucursale
                REPLACE debito WITH debito+(m.creditos-m.debitos)
             ENDIF
          ENDIF
       ENDIF
