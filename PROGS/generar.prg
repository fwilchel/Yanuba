...
*!*	 ENDIF
 ...
 IF m.ventas
    ...
    SELECT vistaventas.fecha, marcas.cuenta, almacenes.idfront AS sucursal, 000000000000000.00  AS debito, precio*unidadespagadas+dto*precio*unidadespagadas/100 AS credito, nit_varios AS tercero, 1, 2 AS naturaleza, 000000000000000.00  AS base, marcas.cc, "INTERFAZ ICG - VALOR ANTES DE IMPUESTOS" AS descripcio, .F. AS vtacredito, 000000 AS numfac, IIF(pu.codformapago='19', 010, almacenes.idfront) AS sucursale, pu.codformapago, cuantos, 01 as orden, SPACE(30) as descripci2 ;
    	...
    	WHERE tipoimpuesto<>3 AND SUBSTR(vistaventas.numserie, 4, 1)<>'I' AND precio*unidadespagadas<>0 AND vistaventas.codarticulo>0 ;
    UNION ALL SELECT vistaventas.fecha, impuestos.cuenta_ventas AS cuenta, almacenes.idfront AS sucursal, 000000000000000.00  AS debito, (total-total*dtocomercial/100)*vistaventas.iva/100 AS credito, nit_varios AS tercero, 2, 2 AS naturaleza, total-total*dtocomercial/100 AS base, "        " AS cc, "INTERFAZ ICG - IMPUESTOS               " AS descripcio, .F. AS vtacredito, 000000 AS numfac, IIF(pu.codformapago='19', 010, almacenes.idfront) AS sucursale, pu.codformapago, cuantos, 02 as orden, "" as descripci2 ;
    ...
    UNION ALL SELECT vistapagos.fecha, formaspago.cuenta, almacenes.idfront AS sucursal, importe AS debito, 000000000000000.00  AS credito, IIF(LEN(ALLTRIM(formaspago.nit))>0, formaspago.nit, IIF(formaspago.credito, ALLTRIM(IIF(ISNULL(cif), "", cif)), nit_varios)) AS tercero, 3, 1 AS naturaleza, 000000000000000.00  AS base, formaspago.cc AS cc, "INTERFAZ ICG - PAGOS                   " AS descripcio, formaspago.credito AS vtacredito, IIF(formaspago.credito, IIF(formaspago.numerofiscal, vistapagos.numerofiscal, getconsecutivo2(vistapagos.fecha)), 000000) AS numfac, IIF((pu.codformapago='19' AND cuantos=1) OR (pu.codformapago='19' AND cuantos>1 AND importe>0), 010, almacenes.idfront) AS sucursale, pu.codformapago, cuantos, 03 as orden, formasPago.descripcion as descripci2 ;
    ...
    UNION ALL SELECT descuentoscomerciales.fecha, m.cuenta_descuentos AS cuenta, almacenes.idfront AS sucursal, SUM(totdtocomercial) AS debito, 000000000000000.00  AS credito, nit_varios AS tercero, 6, 1 AS naturaleza, 000000000000000.00  AS base, "30080   " AS cc, "INTERFAZ ICG - DESCUENTOS              " AS descripcio, .F. AS vtacredito, 000000 AS numfac, IIF(pu.codformapago='19', 010, almacenes.idfront) AS sucursale, pu.codformapago, cuantos, 06 as orden, "" as descripci2 ;
    	...
    	WHERE SUBSTR(descuentoscomerciales.numserie, 4, 1)<>'I' AND totdtocomercial<>0 GROUP BY descuentoscomerciales.fecha, almacenes.idfront, sucursale, pu.codformapago INTO CURSOR ventas1

    SELECT * FROM ventas1 ;
    UNION ALL SELECT vistapropinas.fecha, m.cuenta_credito_propinas AS cuenta, almacenes.idfront AS sucursal, 000000000000000.00  AS debito, propina AS credito, nit_varios AS tercero, 7, 2 AS naturaleza, 000000000000000.00  AS base, "        " AS cc, "INTERFAZ ICG - PROPINAS                " AS descripcio, .F. AS vtacredito, 000000 AS numfac, IIF(pu.codformapago='19', 010, almacenes.idfront) AS sucursale, pu.codformapago, cuantos, 07 as orden, "" as descripci2 ;
    ...
    UNION ALL SELECT vencabezado.fecha, m.cuenta_cr_cree AS cuenta, almacenes.idfront AS sucursal, 000000000000000.00  AS debito, m.tasa_cree*totalbruto/100 AS credito, nit_varios AS tercero, 1, 2 AS naturaleza, 000000000000000.00  AS base, "        " AS cc, "INTERFAZ ICG - AUTORETENCION CREE" AS descripcio, .F. AS vtacredito, 000000 AS numfac, IIF(pu.codformapago='19', 010, almacenes.idfront) AS sucursale, pu.codformapago, cuantos, 12 as orden, "" as descripci2 ;
    	FROM vencabezado LEFT JOIN vistaSucursales ON vencabezado.caja=vistasucursales.cajamanager ;
    	LEFT JOIN almacenes ON SUBSTR(vistasucursales.cajamanager, 1, 2)==almacenes.codalmacen ;
    	LEFT JOIN pagoUnico AS pu ON vencabezado.fecha=pu.fecha AND vencabezado.numserie=pu.serie AND vencabezado.numalbaran=pu.numero INTO CURSOR ventas
