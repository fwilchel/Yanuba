...
*!*	 ENDIF
 ...
 IF m.ventas
    ...
    SELECT DISTINCT fecha, numserie, numalbaran, caja, SUM(precio*unidadespagadas-(precio*unidadespagadas*(1-dto/100)*(1-dtocomercial/100))) AS totdtocomercial ;
    FROM vistaVentas GROUP BY 1, 2, 3, 4 INTO CURSOR descuentosComerciales
 ...
 SELECT vistaventas.fecha, m.cuenta_exentas AS cuenta, almacenes.idfront AS sucursal, 000000000000000.00  AS debito, precio*unidadespagadas AS credito, nit_varios AS tercero, 4, 2 AS naturaleza, 000000000000000.00  AS base, marcas.cc, "INTERFAZ ICG - EXENTOS                 " AS descripcio, .F. AS vtacredito, 000000 AS numfac, IIF(pu.codformapago='19', 010, almacenes.idfront) AS sucursale, pu.codformapago, cuantos, 04 as orden, "" as descripci2, vistaVentas.numserie, vistaventas.numalbaran ;
    	...
    	WHERE tipoimpuesto=3  AND precio*unidadespagadas<>0 AND vistaventas.codarticulo<>3240 INTO CURSOR exentos
    ...
    SELECT vistaventas.fecha, marcas.cuenta, almacenes.idfront AS sucursal, 000000000000000.00 AS debito, precio*unidadespagadas+dto*precio*unidadespagadas/100 AS credito, nit_varios AS tercero, 01 as tipo, 2 AS naturaleza, 000000000000000.00  AS base, marcas.cc, "INTERFAZ ICG - VALOR ANTES DE IMPUESTOS" AS descripcio, .F. AS vtacredito, 000000 AS numfac, IIF(pu.codformapago='19', 010, almacenes.idfront) AS sucursale, pu.codformapago, cuantos, 01 as orden, SPACE(30) as descripci2, vistaVentas.numserie, vistaventas.numalbaran ;
    	...
    	LEFT JOIN almacenes ON SUBSTR(vistasucursales.cajamanager, 1, 2)==almacenes.codalmacen ;
    	LEFT JOIN pagoUnico AS pu ON vistaventas.fecha=pu.fecha AND vistaventas.numserie=pu.serie AND vistaventas.numalbaran=pu.numero;
    	WHERE tipoimpuesto<>3 AND precio*unidadespagadas<>0 AND vistaventas.codarticulo>0 AND vistaventas.codarticulo<>3240; 
    ...
    UNION ALL SELECT vistaventas.fecha, impuestos.cuenta_ventas AS cuenta, almacenes.idfront AS sucursal, 000000000000000.00  AS debito, (total-total*dtocomercial/100)*vistaventas.iva/100 AS credito, nit_varios AS tercero, 2, 2 AS naturaleza, total-total*dtocomercial/100 AS base, "        " AS cc, "INTERFAZ ICG - IMPUESTOS               " AS descripcio, .F. AS vtacredito, 000000 AS numfac, IIF(pu.codformapago='19', 010, almacenes.idfront) AS sucursale, pu.codformapago, cuantos, 02 as orden, "" as descripci2, vistaVentas.numserie, vistaventas.numalbaran ;
    	...
    	LEFT JOIN almacenes ON SUBSTR(vistasucursales.cajamanager, 1, 2)==almacenes.codalmacen ;
    	LEFT JOIN pagoUnico AS pu ON vistaventas.fecha=pu.fecha AND vistaventas.numserie=pu.serie AND vistaventas.numalbaran=pu.numero ;
    	WHERE tipoimpuesto<>3 AND LEN(ALLTRIM(caja))>0 ; 
    UNION ALL SELECT vistapagos.fecha, formaspago.cuenta, almacenes.idfront AS sucursal, IIF(importe>=0,importe,000000000000000.00) AS debito, IIF(importe<0,-importe,000000000000000.00)  AS credito, IIF(LEN(ALLTRIM(formaspago.nit))>0, formaspago.nit, IIF(formaspago.credito, ALLTRIM(IIF(ISNULL(cif), "", cif)), nit_varios)) AS tercero,;
     3, IIF(importe>=0,1,2) AS naturaleza, 000000000000000.00  AS base, formaspago.cc AS cc, "INTERFAZ ICG - PAGOS                   " AS descripcio, formaspago.credito AS vtacredito, IIF(formaspago.credito, IIF(formaspago.numerofiscal, vistapagos.numerofiscal, getconsecutivo2(vistapagos.fecha)), 000000) AS numfac, IIF((pu.codformapago='19' AND cuantos=1) OR (pu.codformapago='19' AND cuantos>1 AND importe>0), 010, almacenes.idfront) AS sucursale, pu.codformapago, cuantos, 03 as orden, ;
     formasPago.descripcion as descripci2, vistaPagos.serie, vistaPagos.numero ;
	...
	SELECT vistapagos.fecha, formaspago.cuenta, almacenes.idfront AS sucursal, IIF(importe>=0,IIF(vistaPagos.codcliente=81627 and vistaPagos.codformapago='24', totalbruto * porcentaje_domicilios/100, 000000000000000.00),000000000000000.00) AS debito, IIF(importe<0,IIF(vistaPagos.codcliente=81627 and vistaPagos.codformapago='24', totalbruto * porcentaje_domicilios/100, 000000000000000.00),000000000000000.00)  AS credito, IIF(LEN(ALLTRIM(formaspago.nit))>0, formaspago.nit, IIF(formaspago.credito, ALLTRIM(IIF(ISNULL(cif), "", cif)), nit_varios)) AS tercero,;
     15 as tipo, IIF(importe>=0,1,2) AS naturaleza, 000000000000000.00  AS base, formaspago.cc AS cc, "INTERFAZ ICG - PAGOS                   " AS descripcio, formaspago.credito AS vtacredito, IIF(formaspago.credito, IIF(formaspago.numerofiscal, vistapagos.numerofiscal, getconsecutivo2(vistapagos.fecha)), 000000) AS numfac, IIF((pu.codformapago='19' AND cuantos=1) OR (pu.codformapago='19' AND cuantos>1 AND importe>0), 010, almacenes.idfront) AS sucursale, pu.codformapago, cuantos, 03 as orden, ;
     formasPago.descripcion as descripci2, vistaPagos.serie, vistaPagos.numero ;
    	FROM vistaPagos LEFT JOIN formasPago ON vistapagos.codformapago=formaspago.codformapago ;
    	LEFT JOIN vistaSucursales ON vistapagos.caja=vistasucursales.cajamanager ;
    	LEFT JOIN almacenes ON SUBSTR(vistasucursales.cajamanager, 1, 2)==almacenes.codalmacen ;
    	...
    	WHERE LEN(ALLTRIM(caja))>0 AND vistapagos.codtipopago IS NOT NULL ;
	UNION ALL ;
    SELECT vistapagos.fecha, m.cuenta_domicilios as cuenta, almacenes.idfront AS sucursal, IIF(importe<0, totalbruto * porcentaje_domicilios/100,000000000000000.00) AS debito, IIF(importe>=0, totalbruto * porcentaje_domicilios/100,000000000000000.00)  AS credito, IIF(LEN(ALLTRIM(formaspago.nit))>0, formaspago.nit, IIF(formaspago.credito, ALLTRIM(IIF(ISNULL(cif), "", cif)), nit_varios)) AS tercero,;
     15 as tipo, IIF(importe>=0,2,1) AS naturaleza, 000000000000000.00  AS base, formaspago.cc AS cc, "INTERFAZ ICG - DOMICILIOS.COM          " AS descripcio, formaspago.credito AS vtacredito, IIF(formaspago.credito, IIF(formaspago.numerofiscal, vistapagos.numerofiscal, getconsecutivo2(vistapagos.fecha)), 000000) AS numfac, IIF((pu.codformapago='19' AND cuantos=1) OR (pu.codformapago='19' AND cuantos>1 AND importe>0), 010, almacenes.idfront) AS sucursale, pu.codformapago, cuantos, 03 as orden, ;
     formasPago.descripcion as descripci2, vistaPagos.serie, vistaPagos.numero ;
    	FROM vistaPagos  ;
    	LEFT JOIN vistaSucursales ON vistapagos.caja=vistasucursales.cajamanager ;
    	LEFT JOIN almacenes ON SUBSTR(vistasucursales.cajamanager, 1, 2)==almacenes.codalmacen ;
    	LEFT JOIN pagoUnico AS pu ON vistapagos.fecha=pu.fecha AND vistapagos.serie=pu.serie AND vistapagos.numero=pu.numero ;
    	LEFT JOIN formasPago ON pu.codformapago=formaspago.codformapago ;
    	WHERE LEN(ALLTRIM(caja))>0 AND vistapagos.codtipopago IS NOT NULL and vistaPagos.codcliente=81627 and vistaPagos.codformapago='24' ;    	
    UNION ALL SELECT * from ventas1 WHERE credito<>0 OR debito<>0 UNION ALL select * from ventas2 WHERE credito<>0 OR debito<>0 ;
 ...
 IF m.puntosVencidos
*!*		 SELECT fecha, m.cta_puntos_cr, 22 AS sucursal, -puntos*100 AS debito, 000000000000000.00 AS credito, idtarjeta AS tercero, 1, 1 AS naturaleza, 000000000000000.00 AS base, SPACE(8) as cc, "INTERFAZ ICG - PUNTOS VENCIDOS         " AS descripcio, .F. AS vtacredito, 000000 AS numfac, 010 AS sucursale, '' as codformapago, 0 as cuantos, 01 as orden, SPACE(30) as descripci2 ;
*!*		    	FROM VistaPuntosVencidos ;
*!*		 UNION ALL  SELECT fecha, m.cta_puntos_db, 22 AS sucursal, 000000000000000.00 AS debito, -puntos*100 AS credito, idtarjeta AS tercero, 2, 2 AS naturaleza, 000000000000000.00 AS base, SPACE(8) as cc, "INTERFAZ ICG - PUNTOS VENCIDOS         " AS descripcio, .F. AS vtacredito, 000000 AS numfac, 010 AS sucursale, '' as codformapago, 0 as cuantos, 01 as orden, SPACE(30) as descripci2 ;
*!*		    	FROM VistaPuntosVencidos INTO CURSOR PUNTOSVENCIDOS
    	
    	
   	SELECT puntosAplicados.fecha, m.cta_puntos_cr as cuenta, almacenes.idFront AS sucursal, puntosAplicados.puntosAplicados AS debito, 000000000000000.00 AS credito, IIF(ISNULL(vistaTarjetas.nif20), SPACE(13), vistaTarjetas.nif20) AS tercero, 1, 1 AS naturaleza, 000000000000000.00 AS base, SPACE(8) as cc, "INTERFAZ ICG - PUNTOS VENCIDOS         " AS descripcio, .F. AS vtacredito, 000000 AS numfac, almacenes.idFront AS sucursale, '' as codformapago, 0 as cuantos, 01 as orden, SPACE(30) as descripci2 ;
	    	FROM puntos INNER JOIN PuntosAplicados ON puntos.idLinea= puntosAplicados.idLinea2 ;
	    	LEFT JOIN almacenes ON SUBSTR(puntosAplicados.caja, 1, 2)==almacenes.codalmacen ;
	    	LEFT JOIN vistaTarjetas ON puntos.idTarjeta = vistaTarjetas.idTarjeta ;
	    	WHERE puntosAplicados.fecha>=m.desde AND puntosAplicados.fecha<=m.hasta AND SUBSTR(puntosAplicados.descripcion, 1, 15)="PUNTOS VENCIDOS";
	 UNION ALL  SELECT puntosAplicados.fecha, m.cta_puntos_db, almacenes.idFront AS sucursal, 000000000000000.00 AS debito, puntosAplicados.puntosAplicados AS credito, IIF(ISNULL(vistaTarjetas.nif20), SPACE(13), vistaTarjetas.nif20) AS tercero, 2, 2 AS naturaleza, 000000000000000.00 AS base, SPACE(8) as cc, "INTERFAZ ICG - PUNTOS VENCIDOS         " AS descripcio, .F. AS vtacredito, 000000 AS numfac, almacenes.idFront AS sucursale, '' as codformapago, 0 as cuantos, 01 as orden, SPACE(30) as descripci2 ;
	    	FROM puntos INNER JOIN PuntosAplicados ON puntos.idLinea= puntosAplicados.idLinea2 ;
	    	LEFT JOIN almacenes ON SUBSTR(puntosAplicados.caja, 1, 2)==almacenes.codalmacen ;
	    	LEFT JOIN vistaTarjetas ON puntos.idTarjeta = vistaTarjetas.idTarjeta ;
	    	WHERE puntosAplicados.fecha>=m.desde AND puntosAplicados.fecha<=m.hasta AND SUBSTR(puntosAplicados.descripcion, 1, 15)="PUNTOS VENCIDOS" ;
		 INTO CURSOR PUNTOSVENCIDOS
	    	
	    	
	    	
    COPY TO puntosVencidos.xls TYPE XLS
    BROWSE NOAPPEND NOEDIT NODELETE NORMAL
    use

   	SELECT puntosAplicados.fecha, m.cta_puntos_cr as cuenta, almacenes.idFront AS sucursal, puntosAplicados.puntosAplicados AS debito, 000000000000000.00 AS credito, m.nit_varios AS tercero, 1, 1 AS naturaleza, 000000000000000.00 AS base, SPACE(8) as cc, "INTERFAZ ICG - PUNTOS VENCIDOS         " AS descripcio, .F. AS vtacredito, 000000 AS numfac, almacenes.idFront AS sucursale, '' as codformapago, 0 as cuantos, 01 as orden, SPACE(30) as descripci2 ;
	    	FROM puntos INNER JOIN PuntosAplicados ON puntos.idLinea= puntosAplicados.idLinea2 ;
	    	LEFT JOIN almacenes ON SUBSTR(puntosAplicados.caja, 1, 2)==almacenes.codalmacen ;
	    	WHERE puntosAplicados.fecha>=m.desde AND puntosAplicados.fecha<=m.hasta AND SUBSTR(puntosAplicados.descripcion, 1, 15)="PUNTOS VENCIDOS";
	 UNION ALL  SELECT puntosAplicados.fecha, m.cta_puntos_db, almacenes.idFront AS sucursal, 000000000000000.00 AS debito, puntosAplicados.puntosAplicados AS credito, m.nit_varios AS tercero, 2, 2 AS naturaleza, 000000000000000.00 AS base, SPACE(8) as cc, "INTERFAZ ICG - PUNTOS VENCIDOS         " AS descripcio, .F. AS vtacredito, 000000 AS numfac, almacenes.idFront AS sucursale, '' as codformapago, 0 as cuantos, 01 as orden, SPACE(30) as descripci2 ;
	    	FROM puntos INNER JOIN PuntosAplicados ON puntos.idLinea= puntosAplicados.idLinea2 ;
	    	LEFT JOIN almacenes ON SUBSTR(puntosAplicados.caja, 1, 2)==almacenes.codalmacen ;
	    	WHERE puntosAplicados.fecha>=m.desde AND puntosAplicados.fecha<=m.hasta AND SUBSTR(puntosAplicados.descripcion, 1, 15)="PUNTOS VENCIDOS" ;
		 INTO CURSOR PUNTOSVENCIDOS
 ...
 IF m.compras
    ...
    SELECT fecha AS fecha, marcas.cuenta_compras, almacenes.idfront AS sucursal, 000000000000000.00  AS base, total AS debito, 000000000000000.00  AS credito, proveedores.nit AS tercero, 1 AS naturaleza, esdevolucion, sufactura, 01, "INTERFAZ ICG - COMPRAS                 " AS descripcio, marcas.cc, vpc.codtipopago, 00.0000  AS tasa, 00 AS factortasa, IIF(vpc.codtipopago="1", almacenes.idfront, 010) AS sucursale ;
    FROM vistaCompras LEFT JOIN vistaArticulos ON vistacompras.codarticulo=vistaarticulos.codarticulo ;
    LEFT JOIN marcas ON vistaarticulos.marca=marcas.codmarca ;
    INNER JOIN almacenes ON ALLTRIM(vistacompras.codalmacen)==ALLTRIM(almacenes.codalmacen) ;
    LEFT JOIN vistaPagosCompras AS vpc ON vistacompras.seriefact=vpc.serie AND vistacompras.numfact=vpc.numero AND vistacompras.nfact=vpc.n ;
    LEFT JOIN proveedores ON vistacompras.codproveedor=proveedores.codproveedor ;
    WHERE destino=1 ;
    UNION ALL ;
    SELECT fecha, m.cuenta_impuestos_compras AS cuenta, almacenes.idfront AS sucursal, total*(100-dtocomercial)/100 AS base, (total*(100-dtocomercial)/100)*iva/100 AS debito, 000000000000000.00  AS credito, proveedores.nit AS tercero, 1 AS naturaleza, esdevolucion, sufactura, 2, "INTERFAZ ICG - IVA                     " AS descripcio, "        " AS cc, vpc.codtipopago, iva AS tasa, 00 AS factortasa, IIF(vpc.codtipopago="1", almacenes.idfront, 010) AS sucursale ;
    FROM vistaCompras INNER JOIN almacenes ON vistacompras.codalmacen=almacenes.codalmacen ;
    LEFT JOIN vistaPagosCompras AS vpc ON vistacompras.seriefact=vpc.serie AND vistacompras.numfact=vpc.numero AND vistacompras.nfact=vpc.n ;
    LEFT JOIN proveedores ON vistacompras.codproveedor=proveedores.codproveedor ;
    WHERE destino=1 AND iva>0 ;
    UNION ALL ;
    SELECT DISTINCT fecha, IIF(vpc.codtipopago="1", m.cuenta_debito_propinas, proveedores.cuenta) AS cuenta, IIF(vpc.codtipopago="1", almacenes.idfront, 010) AS sucursal, 000000000000000.00  AS base, 000000000000000.00  AS debito, totalneto AS credito, proveedores.nit AS tercero, 2 AS naturaleza, esdevolucion, sufactura, 3, "INTERFAZ ICG - NETOS                   " AS descripcio, "        " AS cc, vpc.codtipopago, 00.0000  AS tasa, 00 AS factortasa, IIF(vpc.codtipopago="1", almacenes.idfront, 010) AS sucursale ;
    FROM vistaCompras LEFT JOIN vistaArticulos ON vistacompras.codarticulo=vistaarticulos.codarticulo ;
    INNER JOIN almacenes ON vistacompras.codalmacen=almacenes.codalmacen ;
    LEFT JOIN vistaPagosCompras AS vpc ON vistacompras.seriefact=vpc.serie AND vistacompras.numfact=vpc.numero AND vistacompras.nfact=vpc.n ;
    LEFT JOIN proveedores ON vistacompras.codproveedor=proveedores.codproveedor ;
    WHERE destino=1 ;
    UNION ALL ;
    SELECT vistadescuentoscompras.fecha, conceptosdtos.cuenta_compras AS cuenta, IIF(vpc.codtipopago="1" OR vistadescuentoscompras.coddto=5, buscar(filtro.seriefact, filtro.numfact, filtro.nfact), 010) AS sucursal, ABS(vistadescuentoscompras.importe)*100/VAL(SUBSTR(ALLTRIM(conceptosdtos.nombre), AT(" ", ALLTRIM(conceptosdtos.nombre))+1)) AS base, IIF(conceptosdtos.naturaleza="D", ABS(vistadescuentoscompras.importe), 000000000000000.00 ) AS debito, IIF(conceptosdtos.naturaleza="C", ABS(vistadescuentoscompras.importe), 000000000000000.00 ) AS credito, proveedores.nit AS tercero, IIF(conceptosdtos.naturaleza="D", 1, 2) AS naturaleza, esdevolucion, sufactura, conceptosdtos.codigo+3, "INTERFAZ ICG - "+UPPER(conceptosdtos.nombre) AS descripcio, "        " AS cc, vpc.codtipopago, VAL(SUBSTR(ALLTRIM(conceptosdtos.nombre), AT(" ", ALLTRIM(conceptosdtos.nombre))+1)) AS tasa, ;
    conceptosdtos.factortasa, IIF(vpc.codtipopago="1", buscar(filtro.seriefact, filtro.numfact, filtro.nfact), 010) AS sucursale ;
    FROM filtro INNER JOIN VistaDescuentosCompras ON filtro.seriefact=vistadescuentoscompras.numserie AND filtro.numfact=vistadescuentoscompras.numfactura ;
    LEFT JOIN vistaPagosCompras AS vpc ON vistadescuentoscompras.numserie=vpc.serie AND vistadescuentoscompras.numfactura=vpc.numero ;
    LEFT JOIN proveedores ON vpc.codproveedor=proveedores.codproveedor ;
    LEFT JOIN conceptosDtos ON vistadescuentoscompras.coddto=conceptosdtos.codigo ;
    WHERE vistadescuentoscompras.importe<>0 ;
    UNION ALL ;
    SELECT fecha AS fecha, m.cuenta_compras_dtos, IIF(vpc.codtipopago="1", almacenes.idfront, 010) AS sucursal, 000000000000000.00  AS base, 000000000000000.00  AS debito, totdtocomercial AS credito, proveedores.nit AS tercero, 2 AS naturaleza, esdevolucion, sufactura, 20, "INTERFAZ ICG - DESCUENTO               " AS descripcio, "        " AS cc, vpc.codtipopago, dtocomercial AS tasa, 00 AS factortasa, IIF(vpc.codtipopago="1", almacenes.idfront, 010) AS sucursale ;
    FROM vistaCompras INNER JOIN almacenes ON vistacompras.codalmacen=almacenes.codalmacen ;
    LEFT JOIN vistaPagosCompras AS vpc ON vistacompras.seriefact=vpc.serie AND vistacompras.numfact=vpc.numero AND vistacompras.nfact=vpc.n ;
    LEFT JOIN proveedores ON vistacompras.codproveedor=proveedores.codproveedor ;
    WHERE destino=1 AND totdtocomercial<>0 ;
    INTO CURSOR compras
 ...
 ENDIF
 IF m.puntosVencidos
 	SELECT puntosVencidos
	IF RECCOUNT()>0
       SELECT DISTINCT fecha, sucursal FROM puntosvencidos INTO TABLE consecutivos
       ALTER TABLE consecutivos ADD COLUMN consecutiv N (10)
       SCAN
          REPLACE consecutiv WITH getconsecutivo("consecutivo")
       ENDSCAN

       SELECT "EP" AS tipodocumento, consecutiv AS numerodoc, puntosVencidos.fecha, puntosVencidos.cuenta, puntosVencidos.sucursal, ROUND(SUM(debito), 0) AS debito, ROUND(SUM(credito), 0) AS credito, 0 AS base, tercero, cc, descripcio, vtacredito, numfac, puntosVencidos.sucursale, orden ;
       	FROM puntosVencidos LEFT JOIN consecutivos ON puntosVencidos.fecha=consecutivos.fecha AND puntosVencidos.sucursal=consecutivos.sucursal ;
       	WHERE debito<>0 OR credito<>0 GROUP BY puntosVencidos.fecha, tipodocumento, cuenta, descripcio, tercero, puntosVencidos.sucursal, naturaleza, cc, numfac, puntosVencidos.sucursale, orden ;
       	ORDER BY puntosVencidos.fecha, tipodocumento, numerodoc INTO CURSOR c1
...
*!*	       	ORDER BY ventas.fecha, tipodocumento, numerodoc INTO CURSOR c1
       ...
       SELECT ventas.fecha, ventas.sucursal, ROUND(SUM(debito), 0) AS debito, ROUND(SUM(credito), 0) AS credito, IIF(orden=3,"INTERFAZ ICG - "+descripci2, descripcio) as descripcio, orden, Alma2.nombrealmacen, IIF(orden=5 OR orden=6,4,IIF(debito<>0,2,1)) AS natu ;
	       	...
	       	WHERE (debito<>0 OR credito<>0) AND orden NOT in (9,10, 11, 12) AND tipo<>14 GROUP BY 1,2,5,6,7,8 ;
       UNION ALL ;
