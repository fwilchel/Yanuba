 ...
 m.hasta = m.hastabk
 SELECT vm1.fo, vm1.codempleado, vm1.nomvendedor, vm1.dia, vm1.horain, vm2.horaout, vm1.horas+vm2.horas, vm1.caja, vm1.codalmacen ;
 FROM vm1 LEFT JOIN vm2 ON vm1.codempleado=vm2.codempleado AND vm1.caja=vm2.caja AND TTOD(vm1.horaout)+1=TTOD(vm2.horain) ;
 WHERE (HOUR(vm1.horaout)=23 AND MINUTE(vm1.horaout)=59) AND  NOT (HOUR(vm1.horain)=0 AND MINUTE(vm1.horain)=0) INTO CURSOR cortados
  
 SET FILTER TO ISNULL(horaout)
 BROWSE NOAPPEND NOEDIT NODELETE NORMAL TITLE "Marcaciones sin salida"
 SET FILTER TO
 SELECT * FROM vm1 WHERE  NOT (HOUR(horaout)=23 AND MINUTE(horaout)=59) AND NOT (HOUR(horain)=0 AND MINUTE(horain)=0) ;
 UNION ALL ;
 SELECT * FROM cortados WHERE horaout IS NOT NULL INTO CURSOR marcaciones
 
 SELECT vm.*, calcularHoraEntrada(horain) AS entrada, calcularHoraSalida(horaout) AS salida, (calcularHoraSalida(horaout)-calcularHoraEntrada(horain))/3600 AS tiempo ;
 FROM Marcaciones AS vm INTO CURSOR paso1
 
 SELECT paso1.*, IIF(salida<entrada+IIF(caja='B5' AND DOW(entrada)>=2 AND DOW(entrada)<=5, 10, 8)*3600, salida, entrada+IIF(caja='B5' AND DOW(entrada)>=2 AND DOW(entrada)<=5, 10, 8)*3600) AS finturno ;
 FROM paso1 ORDER BY caja, nomvendedor, entrada INTO CURSOR paso2
 
 COPY TO data\extras DATABASE interfaz
 ALTER TABLE extras ADD COLUMN rn N (10, 3)		&& Recargo nocturno
 ALTER TABLE extras ADD COLUMN rnf N (10, 3)	&& Recargo nocturno festivo
 ALTER TABLE extras ADD COLUMN hed N (10, 3)	&& Horas extras diurnas
 ALTER TABLE extras ADD COLUMN hen N (10, 3)	&& Horas extras nocturnas
 ALTER TABLE extras ADD COLUMN hf N (10, 3)		&& Horas festivas
 ALTER TABLE extras ADD COLUMN hefd N (10, 3)	&& Horas extras festivas diurnas
 ALTER TABLE extras ADD COLUMN hefn N (10, 3)	&& Horas extras festivas nocturnas
 ...
 SELECT codempleado, nomvendedor, codalmacen, SUM(rn), SUM(rnf), SUM(hed), SUM(hen), SUM(hf), SUM(hefd), SUM(hefn), SUM(retardos), SUM(hed)-redondear2(SUM(retardos)/60) AS sum_hed_retardos ;
 FROM extras GROUP BY 1, 2, 3 ORDER BY 3, 2 INTO CURSOR ResumenExtras
 ...
 SELECT codempleado, nomvendedor, codalmacen, SUM(rn+rnf) as rn, SUM(hed), SUM(hen), SUM(hf), SUM(hefd), SUM(hefn), SUM(retardos), SUM(hed)-redondear2(SUM(retardos)/60) AS sum_hed_retardos ;
 FROM extras GROUP BY 1, 2, 3 ORDER BY 3, 2 INTO CURSOR ResumenExtras2
...
PROCEDURE rn
 ...
 IF TTOD(m.entrada)<>TTOD(m.salida)
    IF (MINUTE(m.salida)=0 .OR. MINUTE(m.salida)=15 .OR. MINUTE(m.salida)=30 .OR. MINUTE(m.salida)=45)
       m.tiempo = tiempoInterseccion(m.entrada, m.salida, 21, 24, TTOD(m.entrada))
       IF esfestivo(TTOD(m.entrada))
       ...
       ELSE
          REPLACE rn WITH rn+m.tiempo
       ENDIF
       m.tiempo = tiempointerseccion(m.entrada, m.salida, 21, 24, TTOD(m.salida))
       IF esfestivo(TTOD(m.salida))
          REPLACE rnf WITH rnf+m.tiempo
       ELSE
          REPLACE rn WITH rn+m.tiempo
...
FUNCTION esfestivo
 ...
 ENDIF
ENDFUNC
** Por encima del min 54 la hora de entrada es la siguiente
** Si el minuto está entre 24 y 30 queda minuto 30
** Si el minuto está entre 0 y 24 queda minuto 0
** Si el minuto está entre 30 y 54 queda minuto 30
